...
- Execute reviewed local tool calls only behind explicit approval gates. Status: started with `brigade tools call run` for approved `script` calls, local receipts under `.brigade/tools/runs/`, and `brigade tools run list/show/latest/replay` for receipt review and replay queueing without direct reruns.
