...
- Check local release readiness before publish operations. Status: implemented with `brigade release plan`, `doctor`, `run`, `runs`, `show`, `schema`, and `ci`, collecting work closeout, verification, review, scanner, security, handoff, content-guard, local CI platform deprecation evidence, docs, changelog, roadmap, git-state evidence, and wrapper-facing schema manifests without pushing, tagging, or mutating remotes.
